...
OUTPUT_PATH = (
    ...
    / "results"
    / "metrics"
    / "gradcam_attention_audit_v2.csv"
)
...
SUMMARY_PATH = (
    ...
    / "results"
    / "metrics"
    / "gradcam_attention_audit_v2_summary.txt"
)
...
def build_test_table() -> pd.DataFrame:
    """
    Align v2 validation predictions with the cached
    v2 validation manifest.

    The validation prediction CSV was generated with
    shuffle=False, so exact row-order alignment is
    required before any attention analysis.
    """
    ...
    if len(predictions) != 713:
        raise RuntimeError(
            "Expected 713 v2 validation "
            f"predictions, found "
            f"{len(predictions)}."
        )
    ...
    missing_manifest_columns = (
        required_manifest_columns
        - set(validation_manifest.columns)
    )
    ...
    if not labels_match:
        ...
        raise RuntimeError(
            "V2 prediction rows are not "
            "aligned with cached validation "
            "manifest rows. Audit stopped."
        )

    combined = (
        validation_manifest.copy()
    )
    ...
    if missing_files:
        ...
        raise FileNotFoundError(
            "One or more cached v2 "
            "validation images could "
            "not be found."
        )
