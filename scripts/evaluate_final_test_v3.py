...
def main() -> None:
    ...
    print(
        "MEDISCAN AI — "
        "FINAL ADVANCED V3 HELD-OUT TEST"
    )
    print("=" * 70)
    ...
    if not THRESHOLD_ANALYSIS_FILE.is_file():
        raise FileNotFoundError(
            "V3 threshold analysis not found:\n"
            f"{THRESHOLD_ANALYSIS_FILE}"
        )

    print("\nLoading frozen validation threshold...")
    ...
    print(
        f"Threshold source: validation_youden"
    )
    print(
    ...
    print("\nBuilding held-out test dataset...")
    ...
    metrics = {
        "evaluation_type": (
            "final_advanced_v3_held_out_test"
        ),
        "timestamp": datetime.now().isoformat(),
        "model": "advanced_v3",
        "model_file": str(MODEL_FILE),
        "architecture": "EfficientNetV2B0",
        "data_pipeline": "baseline_v2_cached",
        "threshold": threshold,
    ...
    print("\n" + "=" * 70)
    print(
        "FINAL ADVANCED V3 HELD-OUT TEST RESULTS"
    )
    print("=" * 70)
